...
def invitation_saved(sender, instance: GroupInvitation, created: bool, **kwargs):
    if created and not instance.sent_at:
        ...
        if most_recent_matching_invitations.count() > 0:
            if most_recent_matching_invitations.first().sent_at > (timezone.now() - datetime.timedelta(minutes=int(INVITE_EMAIL_COOLDOWN_PERIOD_MINUTES))):
                logger.info(f"Skipping send of invitation to {instance.destination_email} as there's an outstanding email within the last 5 minutes")
                return
        ...
        if SENDGRID_ENABLED:
            ...
            if http_origin_url.port:
                if http_origin_url.scheme == 'http' and http_origin_url.port == '80':
                    pass
                elif http_origin_url.scheme == 'https' and http_origin_url.port == '443':
                    pass
                else:
